...
use super::{
    ...
    window::{AppWindow, MainWindow, Target, WindowHashMap, WindowIDDHashMap},
};
...
impl ApplicationBuilder {
    ...
    fn platform_config(&self, windowed_context: &ContextWrapper<PossiblyCurrent, Window>) {
        if cfg!(windows) {
            ...
            unsafe {
                match handle {
                    RawWindowHandle::Win32(Win32Handle { hwnd, hinstance: _, .. }) => {
    ...
    fn create_main_window(
        ...
        window_index.entry(AppWindow::RegionSelectorCanvasWindow).or_insert(window_id);
                                    ...
                                    | WindowEvent::MouseInput { .. } => match event {
                                        WindowEvent::CursorMoved { position, .. } => {
                                            ...
                                            if s.mouse_state == ElementState::Pressed {
                                                ...
                                                let mouse_data =
                                                    MouseData { button: s.mouse_btn, position };
                                        ...
                                        WindowEvent::MouseInput { state, button, .. } => {
                                            s.mouse_state = state;
                                            let mouse_data =
                                                MouseData { button, position: s.mouse_prev_pos };
                                            match state {
